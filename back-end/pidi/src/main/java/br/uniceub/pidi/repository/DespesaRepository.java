...
public interface DespesaRepository extends JpaRepository<DespesaModel, Long> {
	...
	@Query(value = "SELECT s FROM DespesaModel s WHERE s.data like %?1%" + "OR c.valor like %?1%"
			+ "OR c.descricao like %?1%", countQuery = "SELECT count(*) FROM DespesaModel s WHERE s.data like %?1%"
					+ "OR c.valor like %?1%" + "OR c.descricao like %?1%", nativeQuery = false)
